...
(function () {
    ...
    try {
        ...
        window.INSTAGRAM_MEDIA_TRACKER = {
                ...
                XMLHttpRequest.prototype.send = function (data) {
                    this.addEventListener('load', function () {
                        if (this.status >= 200 && this.status < 300) {
                            try {
                                if (
                                    this.responseURL.includes('/graphql/query')
                                ) {
                                    ...
                                    this.responseURL.includes('/api/v1/feed/')
                                ) {
                                    const responseData = JSON.parse(
                                        this.responseText,
                                    );
            ...
            storeMediaInfo(mediaItem) {
                ...
                if (this.mediaData[shortcode]) {
                    this.updateExistingMedia(
                        this.mediaData[shortcode],
                        mediaItem,
                    );
                    return;
                }
                ...
                const mediaInfo = {
                    code: shortcode,
                    created_at:
                        mediaItem?.caption?.created_at || mediaItem?.taken_at,
                ...
                if (mediaItem.fb_video_id) {
                    ...
                    console.log(
                        '🆔 fbIdMap 추가 (fb_video_id):',
                        mediaItem.fb_video_id,
                        '→',
                        shortcode,
                    );
                }
        ...
        window.INSTAGRAM_UI_SYSTEM = {
            ...
            init() {
                ...
                console.log(
                    '🎨 Instagram UI System 시작 (Instagram downloader 정확한 방식)',
                );
    ...
    `;
            ...
            findCommentButton(video) {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    const commentButtons = searchArea.querySelectorAll(
                        '[aria-label="댓글"], [aria-label="Comment"], [aria-label*="comment"], [aria-label*="댓글"], svg[aria-label="댓글"], svg[aria-label="Comment"], button[aria-label*="댓글"], button[aria-label*="Comment"]',
                    );
    ...
    `;
      ...
      `;
            ...
            findNumberInArea(searchArea, type, button) {
                ...
                if (button) {
                    ...
                    if (button.parentElement) {
                        ...
                        console.log(
                            `👥 ${type} 버튼의 형제 요소들 (${siblings.length}개):`,
                        );
                        for (
                            let j = 0;
                            j < Math.min(siblings.length, 10);
                            j++
                        ) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        for (const element of textElements) {
                            ...
                            if (text && text.length < 20) {
                                // 짧은 텍스트만 로그
                                const isPattern = this.isNumberPattern(
                                    text,
                                    type,
                                );
                                const elementRect =
                                ...
                                console.log(
                                    `🔍 ${type} 텍스트 "${text}": 패턴=${isPattern}, 거리=${distance.toFixed(
                                        1,
                                    )}px, 태그=${element.tagName}, 클래스="${
                                        element.className
                                    }"`,
                                );
                                ...
                                if (isPattern) {
                                    ...
                                    if (distance < 300) {
                                        ...
                                        console.log(
                                            `⚠️ ${type} 숫자 "${text}" 너무 멀음 (거리: ${distance.toFixed(
                                                1,
                                            )}px)`,
                                        );
                                    }
                        ...
                        if (
                            parentText &&
                            this.isNumberPattern(parentText, type)
                        ) {
                            console.log(
                                `✅ ${type} 부모 요소에서 직접 숫자 발견:`,
                                {
                                    text: parentText,
                                    level: i + 1,
                                },
                        ...
                        if (i <= 2) {
                            ...
                            for (const span of spans) {
                                ...
                                if (
                                    spanText &&
                                    spanText.length <= 10 &&
                                    this.isNumberPattern(spanText, type)
                                ) {
                                    const spanRect =
                ...
                if (currentVideo) {
                    ...
                    for (let i = 0; i < 10; i++) {
                        ...
                        if (
                            ...
                            console.log(
                                `🎯 검색 범위를 현재 포스트 컨테이너로 제한: ${parent.tagName}`,
                            );
                            break;
                        }
                    }
                ...
                console.log(
                    `🔍 검색 대상 요소 수: ${allElements.length} (컨테이너: ${
                        searchContainer === document ? '전체문서' : '현재포스트'
                    })`,
                );
                    ...
                    if (
                        ...
                        if (text && text.length < 20) {
                            ...
                            if (overlappingCount <= 10) {
                                // 처음 10개만 로그 출력
                                console.log(
                                    `🔍 확장 영역 내 요소 ${overlappingCount}:`,
                                    {
                                        tag: element.tagName,
                                        text: text,
                                        position: { x: rect.left, y: rect.top },
                                        isPattern: this.isNumberPattern(
                                            text,
                                            type,
                                        ),
                                    },
                ...
                console.log(
                    `📊 확장 검색 결과: ${overlappingCount}개 요소 중 ${foundNumbers.length}개 숫자 후보 발견`,
                );
                ...
                if (type === 'likes') {
                    console.log(
                        '🔍 좋아요 수 특별 검색 시작 (Instagram 전체 구조 분석)',
                    );
                    ...
                    if (currentVideo) {
                        ...
                        for (let i = 0; i < 15; i++) {
                            ...
                            if (
                                ...
                                console.log(
                                    `🔍 확장 컨테이너에서 ${allTextElements.length}개 요소 검사`,
                                );
                                ...
                                for (const element of allTextElements) {
                                    ...
                                    if (
                                        text &&
                                        text.length <= 10 &&
                                        this.isNumberPattern(text, 'likes')
                                    ) {
                                        const rect =
                                            element.getBoundingClientRect();
                                ...
                                console.log(
                                    `📊 확장 검색에서 ${numberCandidates.length}개 숫자 후보 발견`,
                                );

                                if (numberCandidates.length > 0) {
                                    // 거리순으로 정렬
                                    numberCandidates.sort(
                                    ...
                                    // 가장 가까운 숫자 반환
                                    console.log(
                                        `✅ 확장 검색에서 좋아요 수 발견: "${numberCandidates[0].text}"`,
                                    );
                                ...
                                console.log(
                                    '🔍 Instagram 특별 좋아요 검색 (aria-label, data 속성)',
                                );

                                // 좋아요 관련 aria-label이나 data 속성을 가진 요소들 검색
                                const likeElements =
                                    postContainer.querySelectorAll(
                                        '[aria-label*="like"], [aria-label*="좋아요"], [data-testid*="like"], [title*="like"], [title*="좋아요"]',
                                    );
                                console.log(
                                    `📋 좋아요 관련 속성 요소 ${likeElements.length}개 발견`,
                                );
                                ...
                                for (const element of likeElements) {
                                    ...
                                    if (labelMatch) {
                                        console.log(
                                            `✅ aria-label에서 좋아요 수 발견: "${labelMatch[1]}"`,
                                        );
                                        return labelMatch[1];
                                    }
                                ...
                                for (const element of Array.from(
                                    ...
                                    if (props && typeof props === 'object') {
                                        ...
                                        if (likeMatch) {
                                            console.log(
                                                `✅ React props에서 좋아요 수 발견: "${likeMatch[1]}"`,
                                            );
                                            return likeMatch[1];
                                        }
                                    }
                                ...
                                const commentButton =
                                    this.findCommentButton(currentVideo);
                                let commentButtonArea = null;

                                if (commentButton) {
                                    const commentRect =
                                        commentButton.getBoundingClientRect();
                                    commentButtonArea = {
                                    ...
                                    if (text && isValidNumber) {
                                        ...
                                        if (
                                            ...
                                            if (commentButtonArea) {
                                                ...
                                                if (
                                                    elementCenter.x >=
                                                        commentButtonArea.left &&
                                                    elementCenter.x <=
                                                        commentButtonArea.right &&
                                                    elementCenter.y >=
                                                        commentButtonArea.top &&
                                                    elementCenter.y <=
                                                        commentButtonArea.bottom
                                                ) {
                                                    isNearCommentButton = true;
                                                    console.log(
                                                        `⚠️ 댓글 버튼 근처 숫자 제외: "${text}"`,
                                                    );
                                                }
                                            }
                                ...
                                console.log(
                                    `📊 전체 페이지에서 ${potentialNumbers.length}개 숫자 발견 (댓글 수 제외):`,
                                    potentialNumbers.slice(0, 10).map((n) => ({
                                        text: n.text,
                                        distance: Math.round(n.distance),
                                        position: {
                                            x: Math.round(n.rect.left),
                                            y: Math.round(n.rect.top),
                                        },
                                        element:
                                            n.element.tagName +
                                            '.' +
                                ...
                                if (potentialNumbers.length === 0) {
                                    console.log(
                                        '🔍 좋아요 수가 없는 이유 분석 - 전체 페이지의 모든 숫자 확인',
                                    );
                                    const allNumbers = [];
                                ...
                                if (potentialNumbers.length > 0) {
                                    // 거리가 가까운 순으로 정렬
                                    potentialNumbers.sort(
                                        (a, b) => a.distance - b.distance,
                                    );

                                    // 너무 가깝지 않은 것 중에서 선택 (버튼 자체 텍스트 제외)
                                    const validNumbers =
                                        potentialNumbers.filter(
                                            (n) =>
                                                n.distance > 50 &&
                                                n.distance < 1000,
                                        );
                                    ...
                                    if (validNumbers.length > 0) {
                                        console.log(
                                            `✅ 전체 스캔에서 좋아요 수 추정 (댓글 수 제외됨): "${
                                                validNumbers[0].text
                                            }" (거리: ${Math.round(
                                                validNumbers[0].distance,
                                            )}px)`,
                                        );
                                ...
                                for (const selector of igSorterSelectors) {
                                    ...
                                    console.log(
                                        `🔍 IG Sorter 후보 요소 "${selector}": ${elements.length}개`,
                                    );
                                    ...
                                    for (const element of elements) {
                                        const text =
                                            ...
                                            '';
                                        console.log(
                                            `📋 IG Sorter 요소 내용: "${text.substring(
                                                0,
                                                200,
                                            )}"`,
                                        );
                                        ...
                                        const likeMatches = text.match(
                                            /(\d{1,3}(?:,\d{3})*|\d+[KMk]|\d+\.\d+[KMk])/g,
                                        );
                                        if (likeMatches) {
                                            console.log(
                                                `🎯 IG Sorter에서 숫자 패턴 발견: ${likeMatches.join(
                                                    ', ',
                                                )}`,
                                            );
                                            ...
                                            if (likeMatches.length >= 2) {
                                                console.log(
                                                    `✅ IG Sorter에서 좋아요 수 발견: "${likeMatches[1]}"`,
                                                );
                                                return likeMatches[1]; // 두 번째 숫자가 보통 좋아요 수
                                            } else if (
                                                likeMatches.length >= 1
                                            ) {
                                                console.log(
                                                    `✅ IG Sorter에서 숫자 발견: "${likeMatches[0]}"`,
                                                );
                                ...
                                console.log(
                                    '🔍 페이지에서 IG Sorter 스타일 좋아요 수 직접 검색',
                                );
                                ...
                                for (const element of rightSideElements) {
                                    ...
                                    // 화면 오른쪽에 있고, 짧은 숫자 패턴
                                    if (
                                        rect.right > window.innerWidth - 200 && // 오른쪽 200px 이내
                                        ...
                                        console.log(
                                            `🎯 오른쪽 영역에서 숫자 발견: "${text}" 위치: (${Math.round(
                                                rect.left,
                                            )}, ${Math.round(rect.top)})`,
                                        );
                                        return text;
            ...
            isNumberPattern(text, type) {
                ...
                console.log(
                    `🔍 ${type} 패턴 검사: "${cleanText}" (길이: ${cleanText.length})`,
                );
                ...
                if (cleanText.length > 10) {
                    console.log(
                        `❌ ${type} 텍스트 길이 초과: "${cleanText}" (${cleanText.length}자)`,
                    );
                    return false;
                }
    ...
    `;
      ...
      `;
                ...
                const percentage = `${Math.round(
                    (areaWidth / screenWidth) * 100,
                )}% × ${Math.round((areaHeight / screenHeight) * 100)}%`;
    ...
    `;
                ...
                const corners = [
                    {
                        text: `(${Math.round(area.left)}, ${Math.round(
                            area.top,
                        )})`,
                        pos: 'top: -25px; left: 0;',
                    },
                    {
                        text: `(${Math.round(area.right)}, ${Math.round(
                            area.top,
                        )})`,
                        pos: 'top: -25px; right: 0;',
                    },
                    {
                        text: `(${Math.round(area.left)}, ${Math.round(
                            area.bottom,
                        )})`,
                        pos: 'bottom: -25px; left: 0;',
                    },
                    {
                        text: `(${Math.round(area.right)}, ${Math.round(
                            area.bottom,
                        )})`,
                        pos: 'bottom: -25px; right: 0;',
                    },
                ];
      ...
      `;
                ...
                console.log(`🎯 ${areaName} 시각화 완료:`, {
                    영역: `${Math.round(areaWidth)}×${Math.round(
                        areaHeight,
                    )}px`,
                    비율: percentage,
                    좌표: `(${Math.round(area.left)}, ${Math.round(
                        area.top,
                    )}) ~ (${Math.round(area.right)}, ${Math.round(
                        area.bottom,
                    )})`,
                });
            },
    ...
    `;
            ...
            async handleAnalysisClick(container, video, button) {
                ...
                if (currentlyPlayingVideo && currentlyPlayingVideo !== video) {
                    ...
                    console.log(
                        '🔗 실제 활성 video src:',
                        currentlyPlayingVideo?.src?.substring(0, 100),
                    );
                ...
                try {
                    ...
                    const mediaInfo = await this.extractMediaInfoFromContainer(
                        container,
                        video,
                    );
            ...
            async extractMediaInfoFromContainer(container, video) {
                ...
                const description = await this.extractReelsDescription(
                    currentVideo,
                );

                // 1. Instagram downloader 방식: React Props에서 직접 추출
                const shortcode = this.extractShortcodeFromReactProps(
                    container,
                    video,
                );
                if (shortcode) {
                    console.log(
                        '⚛️ React Props에서 shortcode 추출:',
                        shortcode,
                    );
                    ...
                    if (
                        window.INSTAGRAM_MEDIA_TRACKER &&
                        window.INSTAGRAM_MEDIA_TRACKER.mediaData[shortcode]
                    ) {
                        const mediaData =
                            window.INSTAGRAM_MEDIA_TRACKER.mediaData[shortcode];
                        ...
                        return {
                            url:
                                mediaData.video_versions?.[0]?.url || video.src,
                            shortcode: shortcode,
                            mediaData: mediaData,
                ...
                if (
                    window.INSTAGRAM_MEDIA_TRACKER &&
                    window.INSTAGRAM_MEDIA_TRACKER.mediaData[urlShortcode]
                ) {
                    const mediaData =
                        window.INSTAGRAM_MEDIA_TRACKER.mediaData[urlShortcode];
                ...
                try {
                    ...
                    for (const article of articles) {
                        ...
                        const distance = Math.abs(
                            articleCenter - viewportCenter,
                        );

                        // 화면에 보이는 article이고 중앙에 가장 가까운 것
                        if (rect.top < window.innerHeight && rect.bottom > 0) {
                            console.log(
                                `📏 Article 중심: ${articleCenter}, 거리: ${distance}, 영역: ${rect.top}-${rect.bottom}`,
                            );
                    ...
                    if (targetContainer) {
                        const videosInContainer =
                            targetContainer.querySelectorAll('video');
                        console.log(
                            `🎯 선택된 컨테이너의 비디오 수: ${videosInContainer.length}`,
                        );

                        if (videosInContainer.length === 0) {
                            console.log(
                                '⚠️ 선택된 컨테이너에 비디오 없음, 다른 방법으로 검색',
                            );
                    ...
                    if (!targetContainer) {
                        console.log(
                            '❌ 현재 영상의 컨테이너를 찾을 수 없어 설명 추출을 중단합니다',
                        );
                        return '';
                    }
                    ...
                    const reactDescription =
                        await this.extractDescriptionFromReactProps(
                            targetContainer,
                            targetVideo,
                        );
                    if (reactDescription) {
                    ...
                    const apiDescription =
                        this.extractDescriptionFromAPIData(targetContainer);
                    if (apiDescription) {
                    ...
                    const domDescription =
                        this.extractDescriptionFromDOMStructure(
                            targetContainer,
                        );
                    if (domDescription) {
                    ...
                    console.log(
                        '⚠️ 정확한 방법으로 설명을 찾을 수 없어 추측 방식으로 전환합니다',
                    );
                    ...
                    for (const selector of descriptionSelectors) {
                        const elements =
                            targetContainer.querySelectorAll(selector);
                        console.log(
                            `🔍 "${selector}" 검색 결과: ${elements.length}개 요소`,
                        );
                        ...
                        elements.forEach((element, index) => {
                            ...
                            if (element.innerText) {
                                ...
                                if (
                                    ...
                                    description =
                                        this.extractTextWithLineBreaks(element);
                                } else {
                            ...
                            if (description && description.trim().length > 0) {
                                ...
                                if (
                                    elementRect.top >= containerRect.top - 10 &&
                                    elementRect.bottom <=
                    ...
                    if (validTexts.length > 0) {
                        ...
                        console.log(
                            `📏 설명 길이: ${selectedText.text.length}자`,
                        );
                ...
                try {
                    ...
                    if (window.INSTAGRAM_MEDIA_TRACKER) {
                        const mediaData = Object.values(
                            window.INSTAGRAM_MEDIA_TRACKER.mediaData || {},
                        );

                        for (const media of mediaData) {
                            if (media && media.caption && media.caption.text) {
                                console.log(
                                    '✅ Media Tracker에서 caption 발견:',
            ...
            extractDescriptionFromDOMStructure(container) {
                ...
                try {
                    ...
                    for (const element of allElements) {
                        ...
                        if (
                            ...
                            if (
                                ...
                                !text.includes('.mp3') &&
                                !text.includes('.mp4') && // 파일명 제외
                                !text.includes('daniel.mp3') &&
                                !text.includes('Zamaro') && // 특정 오류 텍스트 제외
                                ...
                                !text.includes('관심도') &&
                                !text.includes('views') &&
                                text.length > text.split(' ').length * 2
                            ) {
                                // 평균 단어 길이가 너무 짧지 않음 (해시태그가 아님을 확인)
                    ...
                    console.log(
                        `🔍 총 ${elementsInContainer}개 요소가 컨테이너에 있음`,
                    );
                    console.log(
                        '📋 설명 후보들:',
                        descriptionCandidates.map((c) => ({
                            text: c.text.substring(0, 60) + '...',
                            length: c.length,
                            relativePos:
                                Math.round(c.relativePosition * 100) + '%',
                        })),
                    );
                    ...
                    if (descriptionCandidates.length > 0) {
                        ...
                        descriptionCandidates.sort((a, b) => {
                            ...
                            const aGoodLength =
                                a.length >= 30 && a.length <= 200;
                            const bGoodLength =
                                b.length >= 30 && b.length <= 200;
                        ...
                        console.log(
                            '✅ 컨테이너 기준 설명 발견:',
                            selectedDescription.text.substring(0, 100),
                        );
                        console.log(
                            `📏 설명 길이: ${
                                selectedDescription.length
                            }자, 상대위치: ${Math.round(
                                selectedDescription.relativePosition * 100,
                            )}%`,
                        );
            ...
            findAccountNearVideo(video) {
                try {
                    ...
                    for (const element of allElements) {
                        ...
                        // 비디오와의 거리 계산
                        const distance =
                            Math.abs(rect.top - videoRect.top) +
                            Math.abs(rect.left - videoRect.left);
            ...
            extractAccountInfo(mediaInfo, container, video) {
                ...
                try {
                    ...
                    for (const pattern of urlPatterns) {
                        const match =
                            currentUrl.match(pattern) || urlPath.match(pattern);
                        if (
                            match &&
                            match[1] &&
                            ![
                    ...
                    if (!accountName) {
                        ...
                        console.log(
                            `🔍 총 ${elementsInArea}개 요소가 왼쪽 하단 영역에 있음`,
                        );
                ...
                try {
                    ...
                    if (container) {
                        ...
                        if (!currentUsername && container.url) {
                            currentUsername = this.extractUsernameFromSource(
                                container.url,
                                'Container URL',
                            );
                        }
                    ...
                    if (!currentUsername) {
                        const possibleUrls = [
                            ...
                            container?.url,
                            video?.closest('article')?._instagramVirtualPost
                                ?.url,
                        ];
                        ...
                        for (const url of possibleUrls) {
                            if (url) {
                                currentUsername =
                                    this.extractUsernameFromSource(
                                        url,
                                        'VirtualPost URL',
                                    );
                    ...
                    if (!currentUsername) {
                        ...
                        for (const link of instagramLinks) {
                            ...
                            if (
                                ...
                                console.log(
                                    `🎯 페이지 링크에서 계정명 발견: ${currentUsername}`,
                                );
                                break;
                            }
                        }
                    ...
                    console.log(
                        `📡 IG Sorter에서 "${currentUsername}" 검색 시작`,
                    );
                    const igSorterData =
                        this.getIGSorterLikesCount(currentUsername);
                    if (igSorterData) {
                        console.log(
                            '✅ IG Sorter에서 데이터 발견:',
                            igSorterData,
                        ...
                        if (
                            ...
                            if (igSorterData.uploadDate) {
                                ...
                                for (const key of uniqueKeys) {
                                    window[`uploadDate_${key}`] =
                                        igSorterData.uploadDate;
                                    console.log(
                                        `📅 키 "${key}"로 업로드 날짜 저장: ${igSorterData.uploadDate}`,
                                    );
                                }
                            }
            ...
            getIGSorterLikesCount(username) {
                console.log(
                    `🔍 IG Sorter에서 "${username}" 계정의 좋아요 수 검색 시작`,
                );
                ...
                try {
                    ...
                    const allElements = document.querySelectorAll('*');
                    console.log(
                        `🔍 전체 페이지 요소 수: ${allElements.length}`,
                    );
                    ...
                    for (const element of allElements) {
                        ...
                        if (text.includes('IG Sorter')) {
                            ...
                            const videoBlocks = text
                                .split('IG Sorter')
                                .filter((block) => block.trim());
                            console.log(
                                `🎬 IG Sorter에서 ${videoBlocks.length}개 영상 블록 발견`,
                            );
                            ...
                            if (videoBlocks.length > 0) {
                                ...
                                const numbers =
                                    firstBlock.match(/\d{1,3}(?:,\d{3})*/g);
                                if (numbers && numbers.length >= 1) {
                                    console.log(
                                        `📍 Fallback: 첫 번째 블록의 좋아요 수 ${numbers[0]}`,
                                    );
                                    return numbers[0];
                                }
                            }
                ...
                try {
                    ...
                    if (typeof source === 'string') {
                        ...
                        if (
                            ...
                            username = source;
                            console.log(
                                `🎯 ${sourceType}에서 단순 계정명 발견: ${username}`,
                            );
                        }
                        ...
                        if (username) {
                            console.log(
                                `🎯 ${sourceType} 객체에서 계정명 발견: ${username}`,
                            );
                        }
                    }
            ...
            showIGSorterAlert(username) {
                try {
                    // 기존 알림이 있으면 제거
                    const existingAlert =
                        document.getElementById('ig-sorter-alert');
                    if (existingAlert) {
      ...
      `;
                    ...
                    console.log(
                        `🚨 IG Sorter 데이터 없음 알림 표시 (계정: ${username})`,
                    );
                } catch (error) {
            ...
            extractCommentsCount(container, video) {
                ...
                try {
                    ...
                    const commentsCount = this.findNumberInArea(
                        searchArea,
                        'comments',
                        commentButton,
                    );
            ...
            async expandDescriptionInContainer(container) {
                ...
                try {
                    const allClickableElements = container.querySelectorAll(
                        'button, span[role="button"], div[role="button"], a',
                    );
            ...
            async expandDescriptionIfNeeded() {
                ...
                try {
                    ...
                    console.log(
                        'ℹ️ "더 보기" 버튼을 찾을 수 없음 (이미 전체 내용이 표시되었을 수 있음)',
                    );
                    return false;
                } catch (error) {
            ...
            extractShortcodeFromContainer(container) {
                ...
                // 2. data-* 속성에서 찾기
                const dataShortcode = container.querySelector(
                    '[data-testid*="shortcode"], [data-shortcode]',
                );
                if (dataShortcode) {
                    const shortcode =
                        dataShortcode.getAttribute('data-shortcode') ||
                        dataShortcode
                            .getAttribute('data-testid')
                            .replace('shortcode-', '');
                    if (shortcode) {
                        console.log(
                            '📊 데이터 속성에서 shortcode 추출:',
                            shortcode,
                        );
                        return shortcode;
                    }
                ...
                const reactFiber =
                    ...
                    Object.keys(container).find((key) =>
            ...
            deepSearchForShortcode(obj, depth = 0, visited = new WeakSet()) {
                ...
                for (const key in obj) {
                    if (
                        ...
                        try {
                            const result = this.deepSearchForShortcode(
                                obj[key],
                                depth + 1,
                                visited,
                            );
            ...
            extractFromReactProps(props) {
                try {
                    ...
                    if (props && props.children) {
                        const mediaData = this.deepSearchForVideoData(
                            props.children,
                        );
                        if (mediaData) {
                            return {
            ...
            deepSearchForVideoData(obj, depth = 0) {
                ...
                for (const key in obj) {
                    if (obj.hasOwnProperty(key)) {
                        const result = this.deepSearchForVideoData(
                            obj[key],
                            depth + 1,
                        );
                        if (result) return result;
            ...
            async processVideoAnalysis(mediaInfo, button) {
                // VideoSaver 인스턴스와 연결
                if (
                    typeof window.videoSaver !== 'undefined' &&
                    window.videoSaver
                ) {
                    ...
                    const correctVideo = this.findVideoByShortcode(
                        mediaInfo.shortcode,
                    );
                    console.log(
                        '🎯 shortcode에 맞는 video element:',
                        correctVideo,
                    );
                    ...
                    console.log(
                        '🚀 VideoSaver에 전달할 enhancedMediaInfo:',
                        enhancedMediaInfo,
                    );
            ...
            async analyzeWithVideoSaver(mediaInfo, button) {
                try {
                    ...
                    console.log(
                        '🎬 analyzeWithVideoSaver에서 받은 actualVideo:',
                        actualVideo,
                    );
                    console.log(
                        '🎬 actualVideo src:',
                        actualVideo?.src?.substring(0, 50),
                    );
                    ...
                    console.log(
                        '🎬 최종 사용할 video:',
                        virtualVideo?.src?.substring(0, 50),
                    );
                    ...
                    if (actualVideo) {
                        ...
                        console.log(
                            '🔗 virtualPost에 currentVideo 첨부:',
                            actualVideo,
                        );
                    }

                    // VideoSaver의 분석 메소드 호출 (서버 연결 에러 처리 포함)
                    await window.videoSaver
                        .performHybridAnalysisWithProgress(
                            virtualPost,
                            virtualVideo,
                            (phase, status) => {
                                console.log(
                                    `📊 분석 진행상황: ${phase} - ${status}`,
                                );
                                if (
                                    phase === 'error' &&
                        ...
                        .catch((error) => {
                            ...
                            // 서버 연결 에러인지 확인
                            if (
                                error.message.includes('서버 연결 실패') ||
            ...
            createVirtualPost(mediaInfo) {
                ...
                if (mediaInfo.description) {
                    ...
                    console.log(
                        '📝 virtualPost에 설명 추가:',
                        mediaInfo.description.substring(0, 100) + '...',
                    );
                }
                ...
                if (accountInfo) {
                    ...
                    // 통합 함수로 계정명 추출
                    const username = this.extractUsernameFromSource(
                        accountInfo,
                        'accountInfo',
                    );
                ...
                if (window.location.href.includes('instagram.com')) {
                    ...
                    console.log(
                        '🌐 virtualPost에 현재 URL 추가:',
                        virtualPost.currentUrl,
                    );
                }

                // 좋아요 수 추출 및 추가 (virtualPost 정보 전달)
                const likesCount = this.extractLikesCount(
                    virtualPost,
                    currentVideo,
                );
                ...
                // 댓글 수 추출 및 추가
                const commentsCount = this.extractCommentsCount(
                    null,
                    currentVideo,
                );
            ...
            destroy() {
                ...
                // 모든 생성된 버튼 제거
                document
                    .querySelectorAll('.analysis-overlay-btn')
                    .forEach((btn) => {
                        btn.remove();
                    });
        ...
        const CONSTANTS = {
            ...
            SELECTORS: {
                INSTAGRAM: {
                    ...
                    VIDEOS: 'video',
                    SAVE_BUTTONS:
                        'svg[aria-label*="저장"], svg[aria-label*="Save"], svg[aria-label*="save"]',
                    AUTHOR: 'a[role="link"]',
        ...
        class ApiClient {
            ...
            async processVideo(data) {
                try {
                    ...
                    const isBatchMode =
                        settings.batchMode && data.platform === 'YOUTUBE';
                    ...
                    const response = await fetch(
                        `${this.serverUrl}/api/process-video`,
                        {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
            ...
            async processVideoBlob(data) {
                try {
                    ...
                    formData.append(
                        'video',
                        data.videoBlob,
                        `${data.platform}_video_${Date.now()}.mp4`,
                    );
                    ...
                    const response = await fetch(
                        `${this.serverUrl}/api/process-video-blob`,
                        {
                            method: 'POST',
                            body: formData,
                        },
                    ...
                    if (result.success !== undefined) {
                        Utils.log(
                            'success',
                            'Video blob processed successfully',
                            result.data,
                        );
                        return result.data;
                    } else {
            ...
            async downloadBlobVideo(blobUrl) {
                try {
                    ...
                    throw new Error(
                        `blob 비디오 다운로드 실패: ${error.message}`,
                    );
                }
            }
                ...
                try {
                    ...
                    if (data && data.aiError && data.aiError.occurred) {
                        ...
                        if (error.type === 'gemini_analysis_failed') {
                            ...
                            else if (error.message.includes('사용량 초과'))
                                icon = '📊';
                            else if (error.message.includes('네트워크'))
                                icon = '🌐';
                            else if (error.message.includes('이미지'))
                                icon = '🖼️';
                            else if (error.message.includes('콘텐츠 정책'))
                                icon = '🛡️';
                            else if (error.message.includes('크기 초과'))
                                icon = '📏';
                            else if (error.message.includes('서비스 오류'))
                                icon = '⚙️';
                        }
        ...
        class UIManager {
            ...
            showNotification(
                ...
                duration = CONSTANTS.TIMEOUTS.NOTIFICATION_DURATION,
            ) {
                // 기존 방식과 동일하게 유지
                if (
            ...
            _createNotification(message, type, duration) {
                ...
                const color =
                    colors[type] || colors[CONSTANTS.NOTIFICATION_TYPES.INFO];

                // 기존 알림들의 개수 확인 (위치 조정용)
                const existingNotifications = document.querySelectorAll(
                    '[data-ai-notification]',
                );
      ...
      `;
        ...
        const checkAISettings = async () => {
            return new Promise((resolve) => {
                chrome.storage.sync.get(['videosaverSettings'], (result) => {
                    ...
                    const useAI =
                        settings.useAI !== undefined ? settings.useAI : true;
                    console.log(
                        '🤖 AI 분석 설정 확인:',
                        useAI ? '활성화됨' : '비활성화됨',
                    );
        ...
        class VideoSaver {
                ...
                if (chrome.storage && chrome.storage.onChanged) {
                    chrome.storage.onChanged.addListener(
                        (changes, namespace) => {
                            if (
                                namespace === 'sync' &&
                                changes[CONSTANTS.STORAGE_KEYS.SETTINGS]
                            ) {
                                const newSettings =
                                    changes[CONSTANTS.STORAGE_KEYS.SETTINGS]
                                        .newValue;
                                ...
                                Utils.log(
                                    ...
                                    {
                                        old: oldSettings?.[
                                            CONSTANTS.SETTINGS.AUTO_ANALYSIS
                                        ],
                                        new: newSettings?.[
                                            CONSTANTS.SETTINGS.AUTO_ANALYSIS
                                        ],
            ...
            async init() {
                ...
                Utils.log(
                    'success',
                    `영상 저장기가 ${this.platform}에서 실행됩니다.`,
                );
            ...
            setup() {
                ...
                Utils.log(
                    'success',
                    'setup() 함수 실행 완료 - Instagram UI System 전용',
                );
            }
            ...
            addVideoButton(videoId) {
                ...
                button.addEventListener('click', () =>
                    this.analyzeYouTubeVideo(videoId, false),
                );
            ...
            async analyzeYouTubeVideo(videoId, isShorts) {
                ...
                try {
                    ...
                    Utils.log('info', 'YouTube 영상 분석 시작', {
                        videoId,
                        url,
                        isShorts,
                    });
                    ...
                    this.uiManager.showNotification(
                        `분석 완료: ${
                            result.analysis?.category || '카테고리 분석됨'
                        }`,
                        'success',
                    );
                } catch (error) {
            ...
            enhanceInstagramSaveButtons() {
                ...
                setTimeout(() => {
                    try {
                        ...
                        Utils.log(
                            'info',
                            `발견된 저장 버튼 수: ${saveButtons.length}`,
                        );
            ...
            processSaveButton(svg, index) {
                ...
                if (
                    !button ||
                    button.hasAttribute('data-video-save-enhanced')
                ) {
                    return;
                }
            ...
            createAnalysisButton(originalButton, post, video) {
                // 이미 분석 버튼이 있는지 확인
                const existingAnalysisButton =
                    originalButton.parentElement?.querySelector(
    ...
    `;
                ...
                analysisButton.addEventListener('click', async (event) => {
                    ...
                    try {
                        ...
                        // 하이브리드 분석 실행
                        await this.performHybridAnalysisWithProgress(
                            post,
                            video,
          ...
          `;
                ...
                try {
                    ...
                    if (buttonContainer) {
                        ...
                        this.createFloatingAnalysisButton(
                            video,
                            analysisButton,
                        );
                    }
            ...
            createFloatingAnalysisButton(video, analysisButton) {
                ...
                const videoContainer =
                    video.closest('div') || video.parentElement;
                if (videoContainer) {
                    videoContainer.style.position = 'relative';
                    videoContainer.appendChild(analysisButton);
                    Utils.log(
                        'info',
                        '분석 버튼이 비디오 위에 floating으로 추가됨',
                    );
                }
            }

            async performVideoAnalysis(post, video) {
                return this.performHybridAnalysisWithProgress(
                    post,
                    video,
                    null,
                );
            ...
            async performHybridAnalysisWithProgress(
                post,
                video,
                ...
                try {
                    ...
                    Utils.log(
                        'error',
                        '❌ 서버에 연결할 수 없습니다. 분석을 중단합니다.',
                        error,
                    );

                    // 사용자에게 알림 표시
                    const uiManager =
                        window.videoSaver?.uiManager || this.uiManager;
                    if (uiManager && uiManager.showNotification) {
                        uiManager.showNotification(
                ...
                if (post && post._instagramDescription) {
                    Utils.log(
                        'info',
                        '📝 Instagram UI System에서 전달된 설명 보존:',
                        post._instagramDescription.substring(0, 100) + '...',
                    );
                ...
                if (post && post._instagramAuthor) {
                    Utils.log(
                        'info',
                        '👤 Instagram UI System에서 전달된 계정 보존:',
                        post._instagramAuthor,
                    );
                ...
                if (post && post._instagramLikes) {
                    Utils.log(
                        'info',
                        '❤️ Instagram UI System에서 전달된 좋아요 수 보존:',
                        post._instagramLikes,
                    );
                ...
                if (post && post._instagramComments) {
                    Utils.log(
                        'info',
                        '💬 Instagram UI System에서 전달된 댓글 수 보존:',
                        post._instagramComments,
                    );
                ...
                if (useGemini) {
                    ...
                    Utils.log(
                        'info',
                        '🔮 Gemini 모드: 빠른 분석 건너뛰고 바로 전체 분석 시작',
                    );
                    if (progressCallback) {
                ...
                this.performFullAnalysisWithProgress(
                    post,
                    video,
                    postUrl,
                    metadata,
            ...
            async performFullAnalysisWithProgress(
                post,
                video,
                postUrl,
                metadata,
                ...
                try {
                    ...
                    if (realVideoUrl && !realVideoUrl.startsWith('blob:')) {
                        ...
                        try {
                            // Instagram 전용 URL 검증
                            if (
                                !realVideoUrl.includes('fbcdn.net') &&
                                !realVideoUrl.includes('cdninstagram.com')
                            ) {
                            ...
                            if (
                                realVideoUrl.includes('bytestart=') ||
                                realVideoUrl.includes('byteend=')
                            ) {
                                ...
                                metadata: {
                                    ...
                                    uploadDate: (() => {
                                        ...
                                        const username = usernameMatch
                                            ...
                                            : metadata.author;
                                        const uploadDate =
                                            window[`uploadDate_${username}`] ||
                                            null;
                                        console.log(
                                            `📅 계정 "${username}"의 업로드 날짜: ${uploadDate}`,
                                        );
                                        return uploadDate;
                                    })(),
                                },
                            ...
                            Utils.log(
                                'success',
                                '🔍 Phase 2 완료 - 전체 분석으로 결과 업데이트됨',
                            );
                        } catch (urlError) {
                            Utils.log(
                                'warn',
                                '🎯 실제 URL 처리 실패, 다중 프레임으로 대체',
                                urlError,
                            );
                        ...
                        Utils.log(
                            'info',
                            '📹 blob URL 감지, 다중 프레임 분석 진행',
                        );

                        const multiFrameData = await this.captureMultipleFrames(
                            video,
                            5,
                        ); // 5프레임
                        ...
                        await this.apiClient.processVideoBlob({
                            ...
                            metadata: {
                                ...
                                uploadDate: (() => {
                                    ...
                                    // metadata.author에서 계정명 추출 (성공 방법)
                                    const usernameMatch = metadata.author.match(
                                        /instagram\.com\/([^\/]+)/,
                                    );
                                    const username = usernameMatch
                                        ? usernameMatch[1]
            ...
            async performQuickAnalysis(video, postUrl, metadata, analysisId) {
                try {
                        ...
                        metadata: {
                            ...
                            uploadDate: (() => {
                                ...
                                const username = usernameMatch
                                    ...
                                    : metadata.author;
                                const uploadDate =
                                    window[`uploadDate_${username}`] || null;
                                console.log(
                                    `📅 계정 "${username}"의 업로드 날짜: ${uploadDate}`,
                                );
                                return uploadDate;
                    ...
                    Utils.log(
                        'success',
                        '⚡ Phase 1 완료 - 빠른 분석 결과 제공됨',
                    );
                } catch (error) {
                    Utils.log('error', '⚡ Phase 1 실패', error);
                    this.uiManager.showNotification(
                        '빠른 분석 실패, 전체 분석으로 진행합니다',
                        'warning',
                    );
                }
            }
            ...
            async extractRealVideoUrl(video) {
                try {
                    Utils.log(
                        'info',
                        '🔍 Instagram 실제 비디오 URL 추출 시작 (Instagram Downloader 방식)',
                    );

                    // 🎯 Instagram Downloader 방식: Media Tracker 우선 사용
                    const mediaInfo =
                        INSTAGRAM_MEDIA_TRACKER.getMediaInfoForCurrentVideo();
                    if (
                        mediaInfo?.video_url &&
                        !mediaInfo.video_url.startsWith('blob:')
                    ) {
                        Utils.log(
                            'info',
                            '🚀 Media Tracker에서 실제 URL 발견:',
                            {
                                shortcode: mediaInfo.code,
                                url:
                                    mediaInfo.video_url.substring(0, 80) +
                                    '...',
                            },
                        );
                    ...
                    if (mediaInfo?.video_url) {
                        Utils.log(
                            'info',
                            '📋 Media Tracker에서 blob URL 발견, 기존 로직으로 처리:',
                    ...
                    const instagramVideoUrl =
                        await this.extractFromInstagramPageData();
                    if (instagramVideoUrl) {
                        Utils.log(
                            'info',
                            '📋 페이지 데이터에서 URL 발견:',
                            instagramVideoUrl.substring(0, 80) + '...',
                        );
            ...
            async extractFromInstagramPageData() {
                try {
                    ...
                    const currentUrl = window.location.href;
                    const reelIdMatch = currentUrl.match(
                        /\/reels\/([A-Za-z0-9_-]+)/,
                    );
                    ...
                    for (const script of scripts) {
                        ...
                        if (currentReelId && content.includes(currentReelId)) {
                            Utils.log(
                                'info',
                                `🎯 Script에서 Reel ID ${currentReelId} 발견`,
                            );
                            // Reel ID 근처에서 video_url 찾기
                            const reelSection = this.extractReelSection(
                                content,
                                currentReelId,
                            );
                            if (reelSection) {
                                Utils.log(
                                    'info',
                                    `📋 Reel 섹션 추출 성공 (길이: ${reelSection.length}자)`,
                                );
                                ...
                                const foundKeywords = videoKeywords.filter(
                                    (keyword) => reelSection.includes(keyword),
                                );
                                Utils.log(
                                    'info',
                                    `🔍 Reel 섹션에서 발견된 비디오 키워드: [${foundKeywords.join(
                                        ', ',
                                    )}]`,
                                );
                                ...
                                for (let i = 0; i < patterns.length; i++) {
                                    ...
                                    if (urlMatch) {
                                        ...
                                        if (
                                            ...
                                            Utils.log(
                                                'info',
                                                `🎉 Reel ID ${currentReelId}에 맞는 비디오 URL 발견!`,
                                            );
                                            return url;
                                        } else {
                                            Utils.log(
                                                ...
                                                `❌ 패턴 ${
                                                    i + 1
                                                } 매칭되었지만 조건 불충족: mp4=${url.includes(
                                                    '.mp4',
                                                )}, not-blob=${!url.startsWith(
                                                    'blob:',
                                                )}, fbcdn=${url.includes(
                                                    'fbcdn.net',
                                                )}, cdninstagram=${url.includes(
                                                    'cdninstagram.com',
                                                )}`,
                                            );
                                        }
                                ...
                                Utils.log(
                                    'warn',
                                    `⚠️ Reel 섹션에서 적합한 비디오 URL을 찾지 못함`,
                                );
                            } else {
                            ...
                            Utils.log(
                                'warn',
                                `⚠️ Script에 Reel ID ${currentReelId}가 포함되지 않음`,
                            );
                        }
                            ...
                            for (const pattern of patterns) {
                                ...
                                if (matches) {
                                    ...
                                    if (
                                        ...
                                        Utils.log(
                                            'warn',
                                            `⚠️ FALLBACK 사용: Reel ID ${currentReelId}에 맞는 URL을 찾지 못해 다른 영상 URL 사용`,
                                        );
                                        Utils.log(
                                            'info',
                                            `📋 발견된 URL: ${url.substring(
                                                0,
                                                80,
                                            )}...`,
                                        );
            ...
            async extractFromNetworkRequests(video) {
                try {
                    ...
                    const videoEntries = entries.filter((entry) => {
                        return (
                            videoPatterns.some((pattern) =>
                                pattern.test(entry.name),
                            ) &&
                            (entry.name.includes('fbcdn.net') ||
                                entry.name.includes('cdninstagram.com')) &&
                    ...
                    if (videoEntries.length > 0) {
                        ...
                        const largestEntry = videoEntries.sort(
            ...
            createClickHandler(post, video) {
                ...
                return async (event) => {
                    ...
                    const isAutoAnalysisEnabled =
                        this.cachedSettings[CONSTANTS.SETTINGS.AUTO_ANALYSIS] ||
                        false;
                    Utils.log(
                        'info',
                        `자동 분석 설정 (캐시됨): ${isAutoAnalysisEnabled}`,
                    );
            ...
            extractInstagramMetadata(post) {
                ...
                try {
                    const author =
                        Utils.safeQuerySelector(
                            post,
                            CONSTANTS.SELECTORS.INSTAGRAM.AUTHOR,
                        )?.textContent || '';
                    const captionElement = Utils.safeQuerySelector(
                        post,
                        CONSTANTS.SELECTORS.INSTAGRAM.CAPTION,
                    );
                    ...
                    if (post._instagramLikes) {
                        ...
                        likes = this.extractLikesCount(
                            post,
                            post._instagramCurrentVideo,
                        );
                        console.log(
                            '❤️ 디버깅 박스 기반 좋아요 수 추출 결과:',
                            likes,
                        );
                    }
                    ...
                    if (post._instagramComments) {
                        ...
                        comments = this.extractCommentsCount(
                            post,
                            post._instagramCurrentVideo,
                        );
                        console.log(
                            '💬 디버깅 박스 기반 댓글 수 추출 결과:',
                            comments,
                        );
                    }
                    ...
                    return {
                        ...
                        channelUrl: author.trim()
                            ? `https://www.instagram.com/${author.trim()}/`
                            : '',
            ...
            observeContentChanges() {
                ...
                window.addEventListener('scroll', () => {
                    ...
                    scrollTimeout = setTimeout(() => {
                        if (
                            now - lastScrollTime >
                            CONSTANTS.TIMEOUTS.SCROLL_MIN_INTERVAL
                        ) {
                            lastScrollTime = now;
                            // ⚠️ 기존 저장 버튼 방식 비활성화
                            // this.enhanceInstagramSaveButtons();
            ...
            async processBlobVideo(
                ...
                Utils.log(
                    'info',
                    'blob URL 감지 - Video Element에서 직접 프레임 캡처 시도',
                );
                ...
                if (!currentVideo) {
                    Utils.log(
                        'info',
                        '❌ currentVideo가 없어서 자동 검색 실행',
                    );
                    currentVideo = this.findCurrentPlayingVideo();
                } else {
                    Utils.log(
                        'info',
                        '🎯 전달받은 currentVideo 사용:',
                        currentVideo?.src?.substring(0, 50),
                    );
                }
                ...
                Utils.log(
                    'info',
                    `타겟 비디오: ${targetVideo ? '발견됨' : '없음'}, src: ${
                        targetVideo?.src?.substring(0, 50) || 'N/A'
                    }`,
                );
                ...
                if (targetVideo) {
                    try {
                        Utils.log(
                            'info',
                            '현재 재생 중인 비디오에서 프레임 캡처 중...',
                        );
                        videoBlob = await this.captureVideoFrame(targetVideo);
                        Utils.log(
                            'info',
                            '✅ Video Element에서 프레임 캡처 성공',
                        );
                    } catch (frameError) {
                        Utils.log(
                            'error',
                            '프레임 캡처 실패, blob URL 다운로드 시도',
                            frameError,
                        );

                        // 프레임 캡처 실패시 blob URL 다운로드 시도
                        try {
                            videoBlob = await this.apiClient.downloadBlobVideo(
                                url,
                            );
                            Utils.log('info', 'Blob URL 다운로드 성공');
                        } catch (blobError) {
                            throw new Error(
                                `비디오 처리 실패: 프레임 캡처(${frameError.message})와 Blob 다운로드(${blobError.message}) 모두 실패`,
                            );
                        }
                    }
                } else {
                    ...
                    try {
                        ...
                        throw new Error(
                            `Video Element를 찾을 수 없고 Blob 다운로드도 실패: ${blobError.message}`,
                        );
                    }
                }
            ...
            findCurrentPlayingVideo() {
                ...
                Utils.log(
                    'info',
                    `페이지에서 발견된 video 요소 수: ${videos.length}`,
                );
                ...
                for (let i = 0; i < videos.length; i++) {
                    ...
                    Utils.log(
                        'info',
                        `Video ${i}: visible=${isVisible}, playing=${isPlaying}, src=${
                            video.src?.substring(0, 30) || 'N/A'
                        }`,
                    );
                ...
                if (videos.length > 0) {
                    const largestVideo = Array.from(videos).reduce(
                        (largest, current) => {
            ...
            async captureVideoFrame(videoElement) {
                try {
                    ...
                    Utils.log(
                        'info',
                        `비디오 상태: paused=${videoElement.paused}, currentTime=${videoElement.currentTime}, readyState=${videoElement.readyState}, videoWidth=${videoElement.videoWidth}, videoHeight=${videoElement.videoHeight}`,
                    );
                    Utils.log(
                        'info',
                        `비디오 src: ${
                            videoElement.src?.substring(0, 50) || 'N/A'
                        }`,
                    );
                    ...
                    Utils.log(
                        'info',
                        `캡처 해상도: ${canvas.width}x${canvas.height}`,
                    );
                        ...
                        canvas.toBlob(
                            (blob) => {
                                if (blob) {
                                    Utils.log(
                                        'info',
                                        `프레임 캡처 완료: ${blob.size} bytes`,
                                    );
                                    resolve(blob);
                                } else {
                                    reject(
                                        new Error('Canvas to Blob 변환 실패'),
                                    );
                                }
                            },
                            'image/jpeg',
        ...
        if (
            ...
            window.refreshVideoSaver = () => {
                ...
                if (
                    window.INSTAGRAM_UI_SYSTEM &&
                    window.INSTAGRAM_UI_SYSTEM.isInitialized
                ) {
            ...
            console.log('📺 YouTube 플랫폼 감지됨 - 채널 분석기 로드');
            ...
            `;
                        ...
                        // 호버 효과
                        this.channelButton.addEventListener(
                            'mouseenter',
                            () => {
                    ...
                    extractChannelInfo() {
                        ...
                        const channelNameEl = document.querySelector(
                            '#channel-name #text, .ytd-channel-name #text, #text-container h1',
                        );
                        ...
                        const subscriberEl = document.querySelector(
                            '#subscriber-count #text, .ytd-subscriber-count #text',
                        );
                        channelInfo.subscribers =
                            subscriberEl?.textContent?.trim() || '';
                    ...
                    async sendAnalysisRequest(data) {
                        const response = await fetch(
                            'http://localhost:3000/api/analyze-channel',
                            {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                    ...
                    updateButtonState(text, isLoading) {
                        ...
                        this.channelButton.style.opacity = isLoading
                            ? '0.7'
                            : '1';
                        this.channelButton.style.cursor = isLoading
                            ? 'not-allowed'
                            : 'pointer';
                    }
                }
    ...
    `;
                ...
                console.error('❌ YouTube 채널 분석기 오류:', ytError);
            }
        } else {
