// YouTube 전용 Content Script
console.log('📺 YouTube Content Script 로드됨');
...
if (window.location.hostname === 'www.youtube.com') {
  ...
  class YouTubeChannelAnalyzer {
    ...
    waitForChannelHeader() {
      ...
      const checkHeader = () => {
        ...
        const subscriberCount = document.querySelector('#subscriber-count, .ytd-subscriber-count, #owner-sub-count, .subscriber-count, [aria-label*="구독자"]');
        
        console.log(`🔍 시도 ${attempts}: 채널명(${!!channelName}), 구독자수(${!!subscriberCount})`);
        
        if (channelName) {
          // 구독자 수가 없어도 채널명만 있으면 버튼 생성 (채널 페이지라는 뜻)
  ...
  try {
    ...
    console.log('✅ YouTube 채널 분석기 전역 변수 설정 완료');
  } catch (error) {
    console.error('❌ YouTube 채널 분석기 초기화 실패:', error);
  }
  
} else {
  console.log('❌ YouTube가 아님:', window.location.hostname);
}

console.log('📺 YouTube Content Script 로드 완료');