...
export class TikTokHandler extends BasePlatformHandler {
    ...
    processVideoContainer(videoContainer, index) {
        ...
        if (!sideActions) {
            this.log(
                'warn',
                `비디오 ${index + 1}: 사이드 액션 영역을 찾을 수 없음`,
            );
            return;
        }

        this.addSaveButtonToSideActions(
            sideActions,
            videoContainer,
            videoElement,
            index,
    ...
    findSideActions(videoContainer) {
        ...
        const videoWrapper = videoContainer.closest(
            CONSTANTS.SELECTORS.TIKTOK.VIDEO_WRAPPER,
        );
        if (videoWrapper) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const potentialSideActions = current.querySelector(
                '[class*="side"], [class*="action"], [class*="button"]',
            );
            if (potentialSideActions) {
                return (
                    potentialSideActions.parentElement || potentialSideActions
                );
            }
    ...
    addSaveButtonToSideActions(
        ...
        try {
            ...
            this.log(
                'success',
                `TikTok 비디오 ${index + 1}: 저장 버튼 추가 완료`,
            );
        } catch (error) {
            this.log(
                'error',
                `TikTok 비디오 ${index + 1}: 버튼 추가 실패`,
                error,
            );
            // 실패 시 플로팅 버튼으로 대체
    ...
    `;
        ...
        button.addEventListener('mouseenter', () => {
            ...
            button.style.background =
                'linear-gradient(45deg, #ff5252, #26c6da) !important';
        });
    ...
    async processBlobVideo(videoUrl, postUrl, metadata, videoElement) {
        ...
        try {
            ...
            this.log(
                'warn',
                'TikTok blob URL 다운로드 실패, Canvas 프레임 캡처로 대체',
                error,
            );

            if (!videoElement) {
                throw new Error(
                    '비디오 요소가 없어서 Canvas 프레임 캡처를 할 수 없습니다.',
                );
            }

            // Canvas를 사용한 프레임 캡처 대안
            const frameBlob = await this.apiClient.captureVideoFrame(
                videoElement,
            );
    ...
    extractMetadata(videoContainer) {
        try {
            ...
            return {
                ...
                likes: likes.trim(),
                commentsCount: '0', // TikTok은 댓글수 추출 어려움
                hashtags: hashtags,
            ...
            for (const selector of dateSelectors) {
                ...
                if (timeElement) {
                    ...
                    if (
                        ...
                        this.log(
                            'info',
                            `TikTok 업로드 날짜 추출 성공 (datetime): ${datetime} -> ${uploadDate}`,
                        );
                        return uploadDate;
                    }
                    ...
                    if (timeElement.title) {
                        try {
                            ...
                            if (!isNaN(parsedDate.getTime())) {
                                ...
                                this.log(
                                    'info',
                                    `TikTok 업로드 날짜 추출 성공 (title): ${timeElement.title} -> ${uploadDate}`,
                                );
                                return uploadDate;
                            }
                        } catch (e) {}
                    ...
                    if (parsedDate) {
                        ...
                        this.log(
                            'info',
                            `TikTok 업로드 날짜 추출 성공 (상대시간): ${timeText} -> ${uploadDate}`,
                        );
                        return uploadDate;
                    }
                }
                ...
                for (const element of elements) {
                    ...
                    if (this.isRelativeTimePattern(text)) {
                        ...
                        if (parsedDate) {
                            ...
                            this.log(
                                'info',
                                `TikTok 업로드 날짜 추출 성공 (패턴매칭): ${text} -> ${uploadDate}`,
                            );
                            return uploadDate;
                        }
                    }
    ...
    parseRelativeDate(timeText) {
        try {
            ...
            this.log('error', 'TikTok 상대적 시간 파싱 실패', {
                timeText,
                error,
            });
            return null;
