  ...
  async extractFromChannel(channel, contentType = 'longform') {
    try {
      ...
      if (this.tagCache.has(cacheKey)) {
        ServerLogger.info('📋 캐시된 태그 사용', { channel: channel.name });
        return this.tagCache.get(cacheKey);
      }

      ServerLogger.info('🤖 AI 태그 추출 시작', { channel: channel.name });
    ...
    try {
      ...
      const contentTypeContext = {
        longform: {
          ...
          keywords: ['심화', '완전정복', '상세분석', '강의', '깊이있는', '전문']
        },
        shortform: {
          description: '숏폼 콘텐츠 (1분 이하 짧은 영상)',
          focus: '빠른 팁, 하이라이트, 요약, 트렌드, 간단 정보',
          keywords: ['팁', '요약', '하이라이트', '트렌드', '빠른', '간단']
        },
        mixed: {
      ...
      const context = contentTypeContext[contentType] || contentTypeContext.longform;
