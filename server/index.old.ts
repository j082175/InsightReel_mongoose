...
import * as SERVER_CONSTANTS from './config/constants';
...
import SheetsManager from './services/SheetsManager';
import UnifiedVideoSaver from './services/UnifiedVideoSaver';
// import youtubeBatchProcessor from './services/YouTubeBatchProcessor';
import HighViewCollector from './services/HighViewCollector';
import YouTubeChannelDataCollector from './services/YouTubeChannelDataCollector';
import { ServerLogger } from './utils/logger';
import ResponseHandler from './utils/response-handler';
import ApiKeyManager from './services/ApiKeyManager';
import UsageTracker from './utils/usage-tracker';
import { API_MESSAGES, ERROR_CODES, HTTP_STATUS_CODES } from './config/api-messages';
...
// 매우 초기 디버그 API 추가
app.get('/api/debug-very-early', (req: Request, res: Response) => {
...
// 서비스 초기화 전 디버그
app.get('/api/debug-before-services', (req: Request, res: Response) => {
    res.status(HTTP_STATUS_CODES.OK).json({
        success: true,
...
// 서비스 초기화 후 디버그
app.get('/api/debug-after-services', (req: Request, res: Response) => {
    res.status(HTTP_STATUS_CODES.OK).json({
        success: true,
...
// 🗄️ MongoDB 전용 헬스 체크
app.get('/api/database/health', async (req: Request, res: Response) => {
    try {
...
// 📊 컬렉션 상태 확인
app.get('/api/database/collections', async (req: Request, res: Response) => {
    try {
...
app.delete('/api/database/reset', async (req: Request, res: Response) => {
    try {
...
// 📊 마이그레이션 진행 상황 조회
app.get('/api/database/migration-status', async (req: Request, res: Response) => {
    try {
...
// Gemini 사용량 통계 조회
app.get('/api/gemini/usage', (req: Request, res: Response) => {
    try {
...
// 구글 시트 연결 테스트
app.get('/api/test-sheets', async (req: Request, res: Response) => {
    try {
...
app.get('/api/test-youtube-sheet', async (req: Request, res: Response) => {
    try {
...
// 🔍 구글 시트 구조 확인 API
app.get('/api/test-sheet-structure', async (req: Request, res: Response) => {
    try {
...
// 🔍 모든 시트별 데이터 수 확인 API
app.get('/api/test-all-sheets-count', async (req: Request, res: Response) => {
    try {
...
app.get('/api/test-instagram-latest', async (req: Request, res: Response) => {
    try {
...
// 설정 상태 확인 API
app.get('/api/config/health', (req: Request, res: Response) => {
    try {
...
app.post('/api/admin/reload-api-keys', (req: Request, res: Response) => {
    try {
...
// 파일 감시 상태 확인 엔드포인트
app.get('/api/admin/file-watcher-status', (req: Request, res: Response) => {
    try {
...
//         ResponseHandler.success(res, result, 'YouTube 배치 강제 처리 완료');
//     } catch (error) {
//         ResponseHandler.error(res, error, '배치 강제 처리 실패');
//     }
// });

// app.post('/api/youtube/batch/add', async (req, res) => {
//     try {
//         const { videoUrl, priority } = req.body;
//         const result = await youtubeBatchProcessor.addToBatch(videoUrl, { priority });
//         ResponseHandler.success(res, result, 'YouTube 배치 큐에 추가 완료');
//     } catch (error) {
//         ResponseHandler.error(res, error, '배치 큐 추가 실패');
//     }
// });
...
//         ResponseHandler.success(res, status, 'YouTube 배치 상태 조회 완료');
//     } catch (error) {
//         ResponseHandler.error(res, error, '배치 상태 조회 실패');
//     }
// });

// app.post('/api/youtube/batch/process-now', async (req, res) => {
//     try {
//         const result = await youtubeBatchProcessor.forceProcess();
//         ResponseHandler.success(res, result, 'YouTube 배치 즉시 처리 완료');
//     } catch (error) {
//         ResponseHandler.error(res, error, '배치 즉시 처리 실패');
//     }
// });
...
//         ResponseHandler.success(res, result, 'YouTube 배치 큐 초기화 완료');
//     } catch (error) {
//         ResponseHandler.error(res, error, '배치 큐 초기화 실패');
//     }
// });
