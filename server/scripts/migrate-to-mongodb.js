...
class DataMigrator {
  ...
  async migrate() {
    try {
      ServerLogger.info('🚀 Google Sheets → MongoDB 마이그레이션 시작', 'MIGRATION');
      ...
      for (const platform of platforms) {
        ServerLogger.info(`📊 ${platform.toUpperCase()} 데이터 처리 시작`, 'MIGRATION');
      ...
      ServerLogger.error('❌ 마이그레이션 전체 실패', error.message, 'MIGRATION');
      throw error;
    }
  }
  ...
  async migratePlatform(platform) {
    try {
      ...
      ServerLogger.info(`📋 ${platform} 시트에서 ${platformData.length}개 행 조회`, 'MIGRATION');
      ...
      for (let i = 0; i < platformData.length; i += batchSize) {
        ...
        ServerLogger.info(`⏳ ${platform} 처리 진행: ${progress}/${platformData.length} (${Math.round(progress/platformData.length*100)}%)`, 'MIGRATION');
      }

    } catch (error) {
      ServerLogger.error(`❌ ${platform} 플랫폼 마이그레이션 실패`, error.message, 'MIGRATION');
  ...
  async processBatch(batch, platform, startIndex) {
    ...
    for (let i = 0; i < batch.length; i++) {
      try {
        ...
        // 데이터 변환
        const videoData = this.transformSheetDataToVideo(rowData, platform);
        ...
        ServerLogger.error(`❌ 행 ${startIndex + i + 2} 처리 실패`, error.message, 'MIGRATION');
        this.migrationStats.errors.push(`Row ${startIndex + i + 2}: ${error.message}`);
      }
    }
    ...
    if (videoDocuments.length > 0) {
      try {
        ...
        ServerLogger.info(`✅ ${videoDocuments.length}개 문서 MongoDB에 삽입 완료`, 'MIGRATION');
      } catch (error) {
        // 부분 실패도 처리
        if (error.writeErrors) {
          const insertedCount = videoDocuments.length - error.writeErrors.length;
          this.migrationStats.successful += insertedCount;
          this.migrationStats.failed += error.writeErrors.length;
          ServerLogger.warn(`⚠️ 배치 부분 삽입: ${insertedCount}개 성공, ${error.writeErrors.length}개 실패`, 'MIGRATION');
        } else {
          this.migrationStats.failed += videoDocuments.length;
          ServerLogger.error(`❌ 배치 삽입 실패`, error.message, 'MIGRATION');
        }
      }
    }
  }
...
${stats.errors.length > 0 ? `❌ 주요 에러: ${stats.errors.slice(0, 5).join(', ')}` : '✅ 에러 없음'}
    `, 'MIGRATION');
...
if (require.main === module) {
  ...
  migrator.migrate()
    ...
    .catch(error => {
      ServerLogger.error('💥 마이그레이션 스크립트 실패', error.message, 'MIGRATION');
      process.exit(1);
    });
}
