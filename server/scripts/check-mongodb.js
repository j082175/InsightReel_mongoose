...
async function checkMongoDB() {
    try {
        ...
        ServerLogger.info(
            `📺 YouTube 비디오: ${youtubeVideos.length}개 (최신 10개)`,
        );
        ...
        if (youtubeVideos.length > 0) {
            ...
            youtubeVideos.forEach((video, index) => {
                ...
                ServerLogger.info(
                    `  📅 생성일: ${
                        video.createdAt?.toLocaleString() || 'N/A'
                    }`,
                );
                ServerLogger.info(
            ...
            ServerLogger.info(
                `  ❌ 핸들명 없음: ${youtubeVideos.length - withHandleCount}개`,
            );
            ...
            ServerLogger.info(
                `  ❌ 채널URL 없음: ${
                    youtubeVideos.length - withChannelUrlCount
                }개`,
            );
        } else {
        ...
        platformStats.forEach((stat) => {
            ServerLogger.info(
                `  ${stat._id}: ${stat.count}개 (핸들명: ${stat.withHandle}개, 채널URL: ${stat.withChannelUrl}개)`,
            );
        });
    } catch (error) {
