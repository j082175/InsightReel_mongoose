...
// 📝 정적 메서드: 채널 등록
channelUrlSchema.statics.registerChannel = async function(normalizedChannelId, originalChannelIdentifier, platform, channelInfo = {}, analysisJob = {}) {
  try {
    ...
    console.log(`✅ 채널 등록 완료 (processing): ${platform} - ${channelInfo.name || originalChannelIdentifier}`);
    if (channelInfo.subscriberCount) {
      console.log(`👥 구독자 수: ${channelInfo.subscriberCount.toLocaleString()}명`);
    }
    ...
    if (error.code === 11000) {
      ...
      return { success: false, error: 'DUPLICATE_CHANNEL', message: '채널이 이미 존재합니다.' };
    }
...
// 🔄 정적 메서드: 상태 업데이트
channelUrlSchema.statics.updateStatus = async function(normalizedChannelId, status, channelInfo = null) {
  try {
    ...
    const result = await this.updateOne({ normalizedChannelId }, updateData);
    
    if (result.modifiedCount > 0) {
      console.log(`✅ 채널 상태 업데이트: ${normalizedChannelId} -> ${status}`);
      return { success: true };
    } else {
      console.warn(`⚠️ 채널 상태 업데이트 실패 (찾을 수 없음): ${normalizedChannelId}`);
...
channelUrlSchema.statics.cleanupStaleProcessing = async function() {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(`🧹 오래된 채널 processing 레코드 정리: ${result.deletedCount}개`);
    }
    
    return { success: true, deletedCount: result.deletedCount };
    
  } catch (error) {
    console.error('오래된 채널 processing 레코드 정리 실패:', error.message);
    return { success: false, error: error.message };
  }
};
