...
interface ProcessVideoBlobRequest {
  platform: string;
  postUrl: string;
...
export class ValidationMiddleware {
   ...
   * 비디오 처리 요청 검증
   */
  static validateProcessVideo(req: Request, res: Response, next: NextFunction): void {
    const { platform, videoUrl, postUrl }: ProcessVideoRequest = req.body;
    ...
    if (errors.length > 0) {
      const error = new Error('입력 데이터 검증 실패') as ValidationError;
   ...
   * 비디오 블롭 처리 요청 검증
   */
  static validateProcessVideoBlob(req: RequestWithFile & Request, res: Response, next: NextFunction): void {
    const { platform, postUrl }: ProcessVideoBlobRequest = req.body;
    const file = req.file;
   ...
   * 파일 업로드 검증
   */
  static validateFileUpload(req: RequestWithFile & Request, res: Response, next: NextFunction): void {
    const file = req.file;

    if (!file) {
      const error = new Error('파일이 업로드되지 않았습니다') as ValidationError;
      error.statusCode = HTTP_STATUS_CODES.BAD_REQUEST;
      error.type = 'VALIDATION_ERROR';
      return next(error);
    }
  ...
  static validateOrigin(req: Request, res: Response, next: NextFunction): void {
    ...
    if (!allowedOrigins.includes(origin)) {
      const error = new Error('허용되지 않는 도메인입니다') as ValidationError;
   ...
   * Content-Type 검증
   */
  static validateContentType(expectedType: string) {
    return (req: Request, res: Response, next: NextFunction): void => {
   ...
   * 메타데이터 검증
   */
  static validateMetadata(req: Request, res: Response, next: NextFunction): void {
