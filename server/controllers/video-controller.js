...
class VideoController {
    ...
    testSheets = ErrorHandler.asyncHandler(async (req, res) => {
        try {
            ...
            res.status(500).json({
                status: 'error',
                message:
                    '구글 시트에 연결할 수 없습니다. API 키와 인증 설정을 확인해주세요.',
    ...
    processVideoBlob = ErrorHandler.asyncHandler(async (req, res) => {
        ...
        ServerLogger.info(
            `📁 File info: ${file.filename} (${file.size} bytes)`,
        );
        ServerLogger.info(
            `🔍 Analysis type: ${analysisType}, AI 분석: ${
                useAI ? '활성화' : '비활성화'
     ...
     * 비디오 처리 파이프라인 실행
     */
    async executeVideoProcessingPipeline({
        platform,
        videoUrl,
        videoPath,
        postUrl,
        ...
        try {
            ...
            if (isBlob && videoPath) {
                ...
                if (platform === PLATFORMS.YOUTUBE) {
                    ...
                    try {
                        const youtubeInfo =
                            await this.videoProcessor.getYouTubeVideoInfo(
                                postUrl,
                            );
                        enrichedMetadata = {
                            ...enrichedMetadata,
                        ...
                        ServerLogger.info(
                            `👤 채널: ${enrichedMetadata.channelName}`,
                        );
                        ServerLogger.info(
                        ...
                        ServerLogger.info(
                            `📅 업로드: ${enrichedMetadata.uploadDate}`,
                        );
                    } catch (error) {
                        ServerLogger.warn(
            ...
            if (analysisType === 'multi-frame' || analysisType === 'full') {
                ...
                pipeline.thumbnailPaths =
                    await this.videoProcessor.generateThumbnail(
                        pipeline.videoPath,
                        analysisType,
                    );
                ServerLogger.info(
                    `✅ ${pipeline.thumbnailPaths.length}개 프레임 추출 완료`,
                );
            } else {
            ...
            if (useAI && analysisType !== 'none') {
                if (pipeline.thumbnailPaths.length > 1) {
                    ServerLogger.info(
                        `3️⃣ 다중 프레임 AI 분석 중... (${pipeline.thumbnailPaths.length}개 프레임)`,
                    );
                } else {
                ...
                pipeline.analysis = {
                    ...
                    hashtags: [],
                    confidence: 0,
                    frameCount: pipeline.thumbnailPaths
    ...
    getSelfLearningStats = ErrorHandler.asyncHandler(async (req, res) => {
        try {
            const stats =
                this.aiAnalyzer.categoryManager.getSelfLearningStats();
            const systemStats =
                this.aiAnalyzer.categoryManager.getSystemStats();
    ...
    uploadTest = ErrorHandler.asyncHandler(async (req, res) => {
        ...
        try {
            const thumbnailPath = await this.videoProcessor.generateThumbnail(
                file.path,
            );

            let analysis = null;
            if (useAI) {
                analysis = await this.aiAnalyzer.analyzeVideo(
                    thumbnailPath,
                    {},
                );
            } else {
