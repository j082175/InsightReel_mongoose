...
class UnifiedCategoryManager {
    ...
    constructor(options = {}) {
        ...
        // 작동 모드 설정
        this.mode =
            options.mode || process.env.CATEGORY_MANAGER_MODE || 'dynamic';
        // 'basic', 'dynamic', 'flexible'

        // 파일 경로들
        this.categoriesPath = path.join(__dirname, '../config/categories.json');
        this.categoriesV2Path = path.join(
            __dirname,
            '../config/categories-v2.json',
        );
        this.normalizationRulesPath = path.join(
            __dirname,
            '../config/normalization-rules.json',
        );
        this.categoryStatsPath = path.join(
            __dirname,
            '../config/category-stats.json',
        );
        this.verifiedCategoriesPath = path.join(
            __dirname,
            '../config/verified-categories.json',
        );
    ...
    loadVerifiedCategories() {
        try {
            if (fs.existsSync(this.verifiedCategoriesPath)) {
                ...
                ServerLogger.success(
                    `검증된 카테고리 로드 완료: ${patternCount}개 패턴`,
                    null,
                    'UNIFIED_CATEGORY',
                );
            } else {
    ...
    getMainCategoriesForPlatform(platform = 'YOUTUBE') {
        // 안전한 문자열 변환
        const platformStr =
            platform && typeof platform === 'string' ? platform : 'YOUTUBE';
        ...
        const categories =
            this.PLATFORM_CATEGORIES[mappedPlatform] ||
            this.PLATFORM_CATEGORIES.youtube;

        ServerLogger.info(
            `📂 플랫폼 '${platform}' → '${mappedPlatform}' 카테고리 ${categories.length}개 반환`,
            null,
            'UNIFIED_CATEGORY',
        );
        ...
        if (category.includes(' > ')) {
            ...
            if (!availableCategories.includes(mainCategory)) {
                return {
                    ...
                    suggestion: `사용 가능한 카테고리: ${availableCategories
                        .slice(0, 3)
                        .join(', ')}...`,
                };
            }
    ...
    validateFlexibleCategory(category) {
        if (!this.categoriesV2) {
            return {
                valid: false,
                message: 'V2 카테고리 데이터가 로드되지 않았습니다',
            };
        }
...
다음 ${
            categories.length
        }개의 대카테고리 중에서 가장 적절한 카테고리를 선택하고, 하위 카테고리를 생성하세요:
...
응답 형식:
{
  ...
  "depth": 4
    ...
    getStatistics() {
        ...
        switch (this.mode) {
            case 'basic':
                stats.categories = this.categories
            ...
            case 'dynamic':
                ...
                stats.totalCategories = Object.values(
                    this.PLATFORM_CATEGORIES,
                ).flat().length;
                stats.verifiedPatterns = Object.keys(
                    this.verifiedCategories,
                ).length;
                stats.normalizationRules = Object.keys(
                    this.normalizationRules,
                ).length;
                break;
            case 'flexible':
                stats.v2Categories = this.categoriesV2
                    ? Object.keys(this.categoriesV2).length
                    : 0;
                break;
        }
            ...
            switch (this.mode) {
                ...
                case 'dynamic':
                    if (this.normalizationRules) {
                        fs.writeFileSync(
                            this.normalizationRulesPath,
                            JSON.stringify(this.normalizationRules, null, 2),
                        );
                    }
                    if (this.categoryStats) {
                        fs.writeFileSync(
                            this.categoryStatsPath,
                            JSON.stringify(this.categoryStats, null, 2),
                        );
                    }
                    if (this.verifiedCategories) {
                        fs.writeFileSync(
                            this.verifiedCategoriesPath,
                            JSON.stringify(this.verifiedCategories, null, 2),
                        );
                    }
                    break;
                case 'flexible':
                    if (this.categoriesV2) {
                        fs.writeFileSync(
                            this.categoriesV2Path,
                            JSON.stringify(this.categoriesV2, null, 2),
                        );
                    }
                    break;
            }
            ServerLogger.success(
                `${this.mode} 모드 설정 저장 완료`,
                null,
                'UNIFIED_CATEGORY',
            );
            return true;
    ...
    switchMode(newMode) {
        if (['basic', 'dynamic', 'flexible'].includes(newMode)) {
            ...
            this.initializeByMode();
            ServerLogger.info(
                `카테고리 관리자 모드 변경: ${oldMode} → ${newMode}`,
                null,
                'UNIFIED_CATEGORY',
            );
            return true;
    ...
    isSelfLearningEnabled() {
        return (
            this.mode === 'dynamic' &&
            process.env.USE_SELF_LEARNING_CATEGORIES === 'true'
        );
    }
    ...
    generateContentSignature(metadata) {
        ...
        if (metadata.caption)
            keywords.push(
                ...metadata.caption.toLowerCase().split(/\s+/).slice(0, 5),
            );
        ...
        if (metadata.title)
            keywords.push(
                ...metadata.title.toLowerCase().split(/\s+/).slice(0, 3),
            );
    ...
    findSimilarVerifiedPattern(contentSignature) {
        ...
        for (const [signature, pattern] of Object.entries(
            this.verifiedCategories.patterns,
        )) {
            ...
            const intersection = searchKeywords.filter((k) =>
                patternKeywordList.includes(k),
            );
            const similarity =
                intersection.length /
                Math.max(searchKeywords.length, patternKeywordList.length);
    ...
    updateVerifiedCategoryUsage(signature) {
        if (
            this.verifiedCategories.patterns &&
            this.verifiedCategories.patterns[signature]
        ) {
            this.verifiedCategories.patterns[signature].lastUsed =
                new Date().toISOString();
            this.verifiedCategories.patterns[signature].usageCount =
                (this.verifiedCategories.patterns[signature].usageCount || 0) +
                1;
        }
    }
    ...
    saveVerifiedCategoryFromAnalysis(contentSignature, analysisResults) {
        ...
        const bestCategory = Object.entries(categoryVotes).sort(
            ([, a], [, b]) => b - a,
        )[0];
        ...
        if (!this.verifiedCategories.patterns) {
            this.verifiedCategories = {
                patterns: {},
                metadata: { totalPatterns: 0 },
            };
        }

        this.verifiedCategories.patterns[contentSignature] = verifiedData;
        this.verifiedCategories.metadata.totalPatterns = Object.keys(
            this.verifiedCategories.patterns,
        ).length;
        ...
        ServerLogger.success(
            `🎯 검증된 카테고리 저장: ${categoryName} (${votes}/${analysisResults.length}표)`,
            null,
            'UNIFIED_CATEGORY',
        );
    ...
    buildVerifiedCategoryReference(similarPattern) {
        ...
        const { verifiedCategory, confidence, analysisCount, similarity } =
            similarPattern;

        return `

**참고할 검증된 카테고리 정보:**
이와 유사한 콘텐츠(유사도: ${(similarity * 100).toFixed(
            1,
        )}%)에서 ${analysisCount}번 분석 결과:
    ...
    processDynamicCategoryResponse(aiResponse, metadata, modelUsed) {
        ...
        try {
            ...
            const availableCategories =
                this.getMainCategoriesForPlatform(platform);

            // 일관성 레벨 확인
            const consistencyLevel =
                parsedResponse.consistency_level || 'medium';
            const consistencyReason =
                parsedResponse.consistency_reason || '일관성 분석 없음';

            // 카테고리 정규화
            let finalCategory =
                parsedResponse.category ||
                parsedResponse.main_category ||
                '미분류';
            ...
            if (consistencyLevel === 'low') {
                ...
                depth = 1;
                ServerLogger.warn(
                    `⚠️ 일관성 부족으로 대카테고리만 지정: ${mainCategory} (${consistencyReason})`,
                );
            ...
            return {
                ...
                keywords: Array.isArray(parsedResponse.keywords)
                    ...
                    : [],
                hashtags: Array.isArray(parsedResponse.hashtags)
                ...
                summary:
                    ...
                    `${mainCategory}/${middleCategory} 카테고리로 분류된 영상입니다.`,
    ...
    getSelfLearningStats() {
        if (this.mode !== 'dynamic') {
            return {
                enabled: false,
                message:
                    '자가 학습 시스템은 dynamic 모드에서만 사용 가능합니다.',
            };
        }
        ...
        return {
            ...
            totalUsage: Object.values(patterns).reduce(
