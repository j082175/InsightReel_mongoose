...
export class VideoDataProcessor {
    private static readonly VIDEO_HEADERS = [
        '제목', '조회수', '채널명', '업로드일', '썸네일',
        '대카테고리', '중카테고리', '소카테고리', '키워드',
        '분석결과', '요약', 'URL', '비디오 길이', '좋아요',
        '댓글수', '구독자수', '채널설명', '태그', '언어', '업로드시간'
    ];

    private static readonly CHANNEL_HEADERS = [
        '채널명', '구독자수', '총 조회수', '비디오 개수',
        '채널 생성일', '채널 설명', '국가', '카테고리',
        '채널 URL', '프로필 이미지', '배너 이미지', '검증 상태'
    ];
        ...
        try {
            ...
            videoDataArray.forEach((videoData, index) => {
                try {
                    ...
                    const errorMsg = `인덱스 ${index} 처리 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`;
    ...
    private static convertVideoToRow(videoData: VideoData, platform: string): any[] {
        ...
        const defaults = {
            ...
            majorCategory: videoData.majorCategory || '',
    ...
    static validateVideoData(videoData: VideoData): { isValid: boolean; errors: string[] } {
        ...
        if (videoData.views !== undefined && (typeof videoData.views !== 'number' || videoData.views < 0)) {
    ...
    static generateDataStats(videoDataArray: VideoData[]): {
        ...
        stats.averageViews = stats.totalVideos > 0 ? Math.floor(stats.totalViews / stats.totalVideos) : 0;
