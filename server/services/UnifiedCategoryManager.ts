...
export class UnifiedCategoryManager {
    ...
    private constructor(options: UnifiedCategoryManagerOptions = {}) {
        this.mode =
            options.mode || (process.env.CATEGORY_MANAGER_MODE as CategoryMode) || "dynamic";
        ...
        try {
            if (fs.existsSync(this.verifiedCategoriesPath)) {
                ...
                if (Array.isArray(parsedData)) {
                    ...
                    this.verifiedCategories = {};
                    ServerLogger.warn(
                        "검증된 카테고리 파일이 배열 형식이 아닙니다. 빈 객체로 초기화",
                        null,
                        "UNIFIED_CATEGORY"
                    );
                }
    ...
    analyzeBasedOnUrl(url: string): CategoryAnalysisResult | null {
        ...
        try {
            ...
            if (this.verifiedCategories[url]) {
                ...
                const [mainCategory, middleCategory] = verified.category.split(" > ");
                return {
                    mainCategory: mainCategory || verified.category,
    ...
    private applyNormalizationRules(url: string): CategoryAnalysisResult | null {
        for (const [ruleId, rule] of Object.entries(this.normalizationRules)) {
            if (
                rule.keywords.some((keyword) => url.toLowerCase().includes(keyword.toLowerCase()))
            ) {
                const [mainCategory, middleCategory] = rule.targetCategory.split(" > ");
                return {
                    mainCategory: mainCategory || rule.targetCategory,
                    middleCategory: middleCategory || "",
    ...
    getCategoriesForPlatform(platform: string): string[] {
        ...
        return (
            this.PLATFORM_CATEGORIES[normalizedPlatform as keyof typeof this.PLATFORM_CATEGORIES] ||
