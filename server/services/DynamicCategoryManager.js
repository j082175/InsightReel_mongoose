...
class DynamicCategoryManager {
    ...
    loadVerifiedCategories() {
        try {
            ...
            ServerLogger.success(
                `검증된 카테고리 로드 완료: ${
                    Object.keys(this.verifiedCategories.patterns).length
                }개 패턴`,
                null,
                'DynamicCategoryManager',
            );
    ...
    normalizeCategoryPath(rawPath, platform = 'YOUTUBE') {
        ...
        if (!validCategories.includes(parts[0])) {
            ServerLogger.warn(
                `유효하지 않은 대카테고리: ${parts[0]} (플랫폼: ${platform})`,
                null,
                'DynamicCategoryManager',
            );
            return null;
    ...
    processDynamicCategoryResponse(aiResponse, metadata, aiModel = null) {
        try {
            ...
            if (!normalized) {
                ServerLogger.warn(
                    `카테고리 정규화 실패: ${categoryData.full_path} (플랫폼: ${platform})`,
                    null,
                    'DynamicCategoryManager',
                );
                return this.getFallbackCategory(metadata);
            ...
            return {
                ...
                keywords: Array.isArray(categoryData.keywords)
                    ? categoryData.keywords.slice(0, 5)
                    : [],
                hashtags: Array.isArray(categoryData.hashtags)
                    ? categoryData.hashtags.slice(0, 5)
                    : [],
    ...
    updateCategoryUsage(categoryPath) {
        ...
        this.categoryStats.usage[categoryPath].lastUsed =
            new Date().toISOString();
        this.categoryStats.lastUpdated = new Date().toISOString();

        // 주기적으로 통계 저장 (매 10회마다)
        if (
            Object.values(this.categoryStats.usage).reduce(
    ...
    learnFromUserFeedback(originalCategory, correctedCategory) {
        ...
        if (original !== corrected) {
            ...
            for (const [key, synonyms] of Object.entries(
                this.normalizationRules.synonyms,
            )) {
            ...
            if (synonymGroup) {
                if (
                    !this.normalizationRules.synonyms[synonymGroup].includes(
                        original,
                    )
                ) {
                    this.normalizationRules.synonyms[synonymGroup].push(
                        original,
                    );
                }
            } else {
    ...
    getMainCategoriesForPlatform(platform) {
        ...
        if (
            ...
            return (
                this.PLATFORM_CATEGORIES[normalizedPlatform] ||
                this.PLATFORM_CATEGORIES.tiktok
            );
        }

        // 기본값은 YouTube 카테고리
        ServerLogger.warn(
            `알 수 없는 플랫폼: ${platform}, YouTube 카테고리 사용`,
            null,
            'DynamicCategoryManager',
        );
        return this.PLATFORM_CATEGORIES.youtube;
    ...
    buildDynamicCategoryPrompt(metadata) {
        ...
        ServerLogger.info(
            `플랫폼: ${platform}, 대카테고리 수: ${categories.length}개`,
            null,
            'DynamicCategoryManager',
        );
...
- 플랫폼: ${metadata.platform || 'unknown'}`;
    ...
    getSystemStats() {
        ...
        const totalUsage = Object.values(this.categoryStats.usage).reduce(
            (sum, stat) => sum + stat.count,
            0,
        );
        const avgDepth =
            Object.keys(this.categoryStats.usage).reduce(
                (sum, path) => sum + path.split(' > ').length,
                0,
            ) / uniquePaths || 0;
        ...
        return {
            ...
            normalizationRules: Object.keys(this.normalizationRules.preferred)
                .length,
            synonymGroups: Object.keys(this.normalizationRules.synonyms).length,
            verifiedPatterns: this.verifiedCategories
                ? Object.keys(this.verifiedCategories.patterns).length
                : 0,
    ...
    findSimilarVerifiedPattern(contentSignature) {
        ...
        for (const [patternSignature, verifiedData] of Object.entries(
            patterns,
        )) {
            const [patternPlatform, patternKeywords] =
                patternSignature.split(':');

            // 플랫폼이 다르면 스킵
            if (patternPlatform !== platform) continue;

            const patternKeywordList = patternKeywords
                .split(',')
                .filter((k) => k.length > 0);

            // 키워드 유사도 계산
            const intersection = currentKeywords.filter((k) =>
                patternKeywordList.includes(k),
            );
            const union = [
                ...new Set([...currentKeywords, ...patternKeywordList]),
            ];
        ...
        if (bestMatch) {
            ServerLogger.info(
                `✅ 유사 패턴 발견: ${bestMatch.signature} (유사도: ${(
    ...
    saveVerifiedCategoryFromAnalysis(contentSignature, analysisResults) {
        ...
        const avgConfidence =
            bestCategory.examples.reduce((sum, ex) => sum + ex.confidence, 0) /
            bestCategory.examples.length;
        ...
        ServerLogger.success(
            `🎯 검증된 카테고리 저장: ${bestCategory.fullPath} (${maxVotes}/${
                validResults.length
            }표, 신뢰도: ${(finalConfidence * 100).toFixed(1)}%)`,
            null,
            'SelfLearning',
        );
...
위 검증된 카테고리를 참고하되, 현재 영상의 실제 내용에 더 적합한 카테고리가 있다면 새로운 분류를 사용하세요.`;
    ...
    updateVerifiedCategoryUsage(contentSignature) {
        if (this.verifiedCategories.patterns[contentSignature]) {
            this.verifiedCategories.patterns[contentSignature].lastUsed =
                new Date().toISOString();
            this.saveVerifiedCategories();
        }
    }
    ...
    getSelfLearningStats() {
        ...
        return {
            ...
            totalVerifications:
                this.verifiedCategories.metadata.totalVerifications || 0,
