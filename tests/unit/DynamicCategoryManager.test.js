...
describe('DynamicCategoryManager', () => {
  ...
  describe('constructor', () => {
    it('기본 설정으로 초기화되어야 함', () => {
      ...
      expect(manager.FIXED_MAIN_CATEGORIES).toEqual(manager.PLATFORM_CATEGORIES.youtube);
    });
  ...
  describe('getMainCategoriesForPlatform', () => {
    ...
    it('Instagram 플랫폼에 대해 12개 카테고리를 반환해야 함', () => {
      const categories = manager.getMainCategoriesForPlatform('instagram');
  ...
  describe('normalizeCategoryPath', () => {
    it('유효한 카테고리 경로를 정규화해야 함', () => {
      const result = manager.normalizeCategoryPath('게임 > 실황 > 호러게임');
    ...
    it('유효하지 않은 대카테고리에 대해 null을 반환해야 함', () => {
      const result = manager.normalizeCategoryPath('잘못된카테고리 > 서브카테고리');
  ...
  describe('processDynamicCategoryResponse', () => {
    it('유효한 AI 응답을 처리해야 함', () => {
      ...
      const result = manager.processDynamicCategoryResponse(aiResponse, {});
    ...
    it('JSON 문자열 응답을 파싱해야 함', () => {
      const jsonString = '{"full_path": "음악 > 커버", "content": "노래 커버 영상"}';

      const result = manager.processDynamicCategoryResponse(jsonString, {});
    ...
    it('유효하지 않은 응답에 대해 폴백 카테고리를 반환해야 함', () => {
      ...
      const result = manager.processDynamicCategoryResponse(invalidResponse, metadata);
    ...
    it('파싱 에러 발생 시 폴백 카테고리를 반환해야 함', () => {
      ...
      const result = manager.processDynamicCategoryResponse(invalidJson, metadata);
  ...
  describe('updateCategoryUsage', () => {
    it('새로운 카테고리 경로의 사용 통계를 생성해야 함', () => {
      ...
      expect(manager.categoryStats.usage[categoryPath].firstUsed).toBeDefined();
      expect(manager.categoryStats.usage[categoryPath].lastUsed).toBeDefined();
    });
