...
describe('VideoProcessor', () => {
  ...
  describe('downloadVideo', () => {
    it('성공적으로 비디오를 다운로드해야 함', async () => {
      ...
      const result = await videoProcessor.downloadVideo('http://example.com/video.mp4', 'instagram');
    ...
    it('blob URL에 대해 에러를 발생시켜야 함', async () => {
      ...
      await expect(videoProcessor.downloadVideo(blobUrl, 'instagram')).rejects.toThrow(
        'Blob URL은 클라이언트에서 파일로 전송해주세요'
      );
    });
    ...
    it('다운로드 실패 시 적절한 에러를 발생시켜야 함', async () => {
      ...
      await expect(videoProcessor.downloadVideo('http://example.com/video.mp4', 'instagram'))
    ...
    it('스트림 쓰기 에러를 처리해야 함', async () => {
      const mockResponse = {
        ...
        data: {
          pipe: jest.fn((writer) => {
            setTimeout(() => writer.emit('error', new Error('Write error')), 50);
          })
        }
      };
  ...
  describe('generateThumbnail', () => {
    it('quick 모드에서 단일 썸네일을 생성해야 함', async () => {
      ...
      videoProcessor.detectFileType = jest.fn().mockResolvedValue('video');
      videoProcessor.generateSingleThumbnail = jest.fn().mockResolvedValue(['/path/to/thumbnail.jpg']);

      const result = await videoProcessor.generateThumbnail('/path/to/video.mp4', 'quick');

      expect(result).toEqual(['/path/to/thumbnail.jpg']);
      expect(videoProcessor.generateSingleThumbnail).toHaveBeenCalledWith('/path/to/video.mp4');
    });

    it('multi-frame 모드에서 다중 프레임을 생성해야 함', async () => {
      // detectFileType 메서드 모킹
      videoProcessor.detectFileType = jest.fn().mockResolvedValue('video');
      videoProcessor.generateMultipleFrames = jest.fn().mockResolvedValue([
        '/path/to/frame1.jpg',
        '/path/to/frame2.jpg',
        '/path/to/frame3.jpg'
      ]);

      const result = await videoProcessor.generateThumbnail('/path/to/video.mp4', 'multi-frame');

      expect(result).toHaveLength(3);
      expect(videoProcessor.generateMultipleFrames).toHaveBeenCalledWith('/path/to/video.mp4');
    });

    it('이미지 파일인 경우 원본을 복사해야 함', async () => {
      videoProcessor.detectFileType = jest.fn().mockResolvedValue('image');

      const result = await videoProcessor.generateThumbnail('/path/to/image.jpg', 'quick');
    ...
    it('썸네일 생성 실패 시 에러를 전파해야 함', async () => {
      videoProcessor.detectFileType = jest.fn().mockRejectedValue(new Error('File type detection failed'));

      await expect(videoProcessor.generateThumbnail('/path/to/video.mp4', 'quick'))
  ...
  describe('generateSingleThumbnail', () => {
    it('FFmpeg으로 성공적으로 썸네일을 생성해야 함', async () => {
      ...
      const result = await videoProcessor.generateSingleThumbnail('/path/to/video.mp4');
    ...
    it('FFmpeg 실행 실패 시 에러를 발생시켜야 함', async () => {
      ...
      await expect(videoProcessor.generateSingleThumbnail('/path/to/video.mp4'))
  ...
  describe('detectFileType', () => {
    ...
    it('이미지 파일을 올바르게 감지해야 함', async () => {
      const result = await videoProcessor.detectFileType('/path/to/image.jpg');
      expect(result).toBe('image');
    });

    it('비디오 파일을 올바르게 감지해야 함', async () => {
      const result = await videoProcessor.detectFileType('/path/to/video.mp4');
      expect(result).toBe('video');
    });

    it('알 수 없는 파일 타입을 처리해야 함', async () => {
      const result = await videoProcessor.detectFileType('/path/to/file.unknown');
  ...
  describe('error handling', () => {
    it('존재하지 않는 파일에 대해 적절한 에러를 발생시켜야 함', async () => {
      videoProcessor.detectFileType = jest.fn().mockRejectedValue(
        new Error('ENOENT: no such file or directory')
      );

      await expect(videoProcessor.generateThumbnail('/nonexistent/file.mp4'))
    ...
    it('디렉토리 생성 실패 시 에러를 발생시켜야 함', () => {
      ...
      expect(() => new VideoProcessor()).toThrow('EACCES: permission denied');
    });
  });
});