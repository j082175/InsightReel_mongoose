...
describe('API Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    app.post('/api/process-video', async (req, res) => {
      try {
        ...
        const thumbnailPaths = await videoProcessor.generateThumbnail(videoPath);
  ...
  describe('POST /api/process-video', () => {
    it('비디오 처리가 성공하면 200을 반환해야 함', async () => {
      // Mock 반환값 설정
      mockVideoProcessor.downloadVideo.mockResolvedValue('/path/to/video.mp4');
      mockVideoProcessor.generateThumbnail.mockResolvedValue(['/path/to/thumb.jpg']);
