...
const BulkCollectionModalTestPage: React.FC = () => {
  ...
  const [collectionHistory, setCollectionHistory] = useState<Array<{
  ...
  // 테스트용 채널 데이터
  const testChannels = [
    { id: 'UC-tech-1', name: '노마드 코더', platform: 'YOUTUBE', category: '테크' },
    { id: 'UC-tech-2', name: '생활코딩', platform: 'YOUTUBE', category: '테크' },
    { id: 'UC-food-1', name: '백종원의 요리비책', platform: 'YOUTUBE', category: '요리' },
    { id: 'UC-food-2', name: '쯔양', platform: 'YOUTUBE', category: '요리' },
    { id: 'IG-kpop-1', name: 'BTS Instagram', platform: 'INSTAGRAM', category: 'K-POP' },
    { id: 'IG-kpop-2', name: 'BLACKPINK Instagram', platform: 'INSTAGRAM', category: 'K-POP' },
    { id: 'TT-dance-1', name: 'Dance Trends TikTok', platform: 'TIKTOK', category: '댄스' },
    ...
    { id: 'TT-comedy-1', name: 'Comedy TikTok', platform: 'TIKTOK', category: '코미디' },
    { id: 'IG-fashion-1', name: 'Fashion Influencer', platform: 'INSTAGRAM', category: '패션' }
  ];
  ...
  const handleOpenModal = (scenario: typeof testScenarios[0]) => {
    ...
    setIsModalOpen(true);
    addTestLog(`대량 수집 시작: ${scenario.name} (${scenario.channels.length}개 채널)`);
  };
  ...
  const totalVideos = collectionHistory.reduce((sum, entry) => sum + entry.videos.length, 0);
  const platformStats = testChannels.reduce((acc, ch) => {
      ...
      <div className="container mx-auto p-8">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <section className="bg-white p-6 rounded-lg border border-gray-200">
            <h2 className="text-xl font-bold text-gray-900 mb-4">📈 테스트 통계</h2>

            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              <div className="bg-gray-50 p-3 rounded text-center">
                <div className="text-lg font-bold text-gray-800">{testChannels.length}</div>
                <div className="text-xs text-gray-600">테스트 채널</div>
              </div>
              <div className="bg-blue-50 p-3 rounded text-center">
                <div className="text-lg font-bold text-blue-700">{platformStats.YOUTUBE || 0}</div>
                <div className="text-xs text-blue-600">YouTube</div>
              </div>
              <div className="bg-pink-50 p-3 rounded text-center">
                <div className="text-lg font-bold text-pink-700">{platformStats.INSTAGRAM || 0}</div>
                <div className="text-xs text-pink-600">Instagram</div>
              </div>
              <div className="bg-black bg-opacity-10 p-3 rounded text-center">
                <div className="text-lg font-bold text-gray-800">{platformStats.TIKTOK || 0}</div>
                <div className="text-xs text-gray-600">TikTok</div>
              </div>
              <div className="bg-green-50 p-3 rounded text-center">
                <div className="text-lg font-bold text-green-700">{totalCollections}</div>
          ...
          <section className="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div>
                <h3 className="font-semibold text-gray-800 mb-3">플랫폼별 선택</h3>
                <div className="space-y-2">
                  <button
          ...
          {/* 수집 시나리오 */}
          <section className="bg-white p-6 rounded-lg border border-gray-200">
            <h2 className="text-xl font-bold text-gray-900 mb-4">🎬 수집 시나리오</h2>
          ...
          {collectionHistory.length > 0 && (
            <section className="bg-white p-6 rounded-lg border border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">📋 수집 히스토리</h2>
                ...
                {collectionHistory.map((entry, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h3 className="font-semibold text-gray-800">{entry.batch.name}</h3>
                        <p className="text-sm text-gray-600">{entry.batch.description || '설명 없음'}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3 text-sm">
                      <div>
                        ...
                        <span className="ml-2 font-medium text-green-600">{entry.videos.length}개</span>
                      </div>
                      <div>
                        <span className="text-gray-500">총 발견:</span>
                        <span className="ml-2 font-medium text-blue-600">{entry.batch.totalVideosFound}개</span>
                      </div>
                      <div>
                        <span className="text-gray-500">할당량:</span>
                        <span className="ml-2 font-medium text-orange-600">{entry.batch.quotaUsed}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">상태:</span>
                        <span className={`ml-2 font-medium ${
                          entry.batch.status === 'completed' ? 'text-green-600' :
                          entry.batch.status === 'failed' ? 'text-red-600' :
                          'text-yellow-600'
                        }`}>
                          {entry.batch.status}
                        </span>
            ...
            <div className="bg-gray-50 p-4 rounded h-64 overflow-y-auto">
              {testActions.length === 0 ? (
                <p className="text-gray-500 text-sm italic">
                  수집 시나리오를 선택하거나 모달과 상호작용하면 로그가 여기에 표시됩니다.
                </p>
              ) : (
          ...
          <section className="bg-blue-50 p-6 rounded-lg border border-blue-200">
            ...
            <div className="space-y-3 text-sm text-blue-800">
              <p>
                <strong>1. 시나리오 선택:</strong> 다양한 수집 시나리오 중 하나를 선택해 모달을 열어보세요.
              </p>
              <p>
                <strong>2. 필터 설정:</strong> 모달에서 기간, 조회수, 영상 길이, 키워드 등을 설정하세요.
              </p>
              <p>
                <strong>3. 진행 모니터링:</strong> 채널별 수집 진행 상황과 실시간 통계를 확인하세요.
              </p>
              <p>
                <strong>4. 에러 확인:</strong> 일부 채널에서 오류가 발생했을 때의 처리를 확인하세요.
              </p>
              <p>
                <strong>5. 결과 분석:</strong> 수집 완료 후 플랫폼별, 길이별 통계를 분석하세요.
              </p>
              <p>
                <strong>6. 대량 처리:</strong> "모든 채널" 시나리오로 대량 수집 성능을 테스트하세요.
              </p>
            </div>
          </section>
