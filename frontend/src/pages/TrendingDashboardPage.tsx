...
const TrendingDashboardPage: React.FC = () => {
  const [trendingStats, setTrendingStats] = useState<TrendingStats | null>(
    null
  );
  ...
  return (
    <div className="p-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              {trendingStats.byPlatform.map((item) => {
                const total = trendingStats.byPlatform.reduce(
                  (sum, p) => sum + p.count,
                  0
                );
                const percentage =
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              {trendingStats.byDuration.map((item) => {
                const total = trendingStats.byDuration.reduce(
                  (sum, d) => sum + d.count,
                  0
                );
                const percentage =
      ...
      {/* 빈 데이터 상태 */}
      {!loading &&
        (!trendingStats || trendingStats.overview.totalVideos === 0) && (
