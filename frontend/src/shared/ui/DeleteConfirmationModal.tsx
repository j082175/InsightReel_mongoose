...
interface DeleteConfirmationModalProps<T = any> {
  itemToDelete: {
    type: 'single' | 'bulk';
    data?: T;
    count?: number;
...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({
  ...
  const message = isBulkDelete
    ? `선택한 ${itemToDelete.count}개의 항목을 삭제하시겠습니까?`
    : `"${(itemToDelete.data as any)?.title || (itemToDelete.data as any)?.name || '선택한 항목'}"을(를) 삭제하시겠습니까?`;
