...
const fetchAPIStatus = async (): Promise<ProcessedAPIStatus> => {
  ...
  const data: { success: boolean; data: APIStatusResponse } =
    await response.json();
  ...
  const activeKey =
    quota.allKeys?.find((key: any) => key.percentage > 0) || quota.allKeys?.[0];

  // 사용 가능한 키 개수 (활성화되고 사용량이 80% 이하인 키들)
  const availableKeys =
    quota.allKeys?.filter(
      (key: any) => key.realStatus !== 'inactive' && key.percentage < 80
    ).length || 0;
  ...
  return {
    ...
    totalUsage: {
      ...
      usagePercentage:
        quota.used && usedLimit > 0
          ? Math.round((quota.used / usedLimit) * 100)
          : 0,
    },
    currentKey: {
